...
### Data set up
source_data("https://github.com/lexcomber/SematicsOfStats/blob/master/rc.RData?raw=True")
...
setwd("/Users/geoaco/Desktop/my_docs_mac/leeds_work/research/COSIT/paperanal/new_anal_v3/new_figs")
...
plot.func.fixed4 <- function(r = r.stand, var = "layer.1", tit = "y", breaks = NULL) {
	p <- tm_shape(r)+
		tm_raster(col = var, alpha = 1, 
			breaks = breaks,
     		title = tit, palette = "RdBu") +
     tm_layout(
     	legend.title.size=1,
     	legend.text.size = 0.6,
     	legend.bg.color = "white",
...
for (i in 1:dim(r.stand)[3]) {
	if (i == 2) p <- plot.func.fixed(r.stand, var = names(r.stand)[i], tit = tits[i], 
		breaks = b.ystar)
	if (i == 4) p <- plot.func.fixed4(r.stand, var = names(r.stand)[i], tit = tits[i], 
		breaks = b.error)
	if( i==1 | i == 3 | i == 5) 
...
for (i in 1:dim(r.x)[3]) {
	if (i == 2) p <- plot.func.fixed(r.x, var = names(r.x)[i], tit = tits[i], 
		breaks = b.ystar)
	if (i == 5) p <- plot.func.fixed4(r.x, var = names(r.x)[i], tit = tits[i], 
		breaks = b.error)
	if( i==1 | i == 3 | i == 4) 
...
for (i in 1:dim(r.v)[3]) {
	if (i == 2) p <- plot.func.fixed(r.v, var = names(r.v)[i], tit = tits[i], 
		breaks = b.ystar)
	if (i == 4) p <- plot.func.fixed4(r.v, var = names(r.v)[i], tit = tits[i], 
		breaks = b.error)
	if( i==1 | i == 3 | i == 5) 
...
for (i in 1:dim(r.m)[3]) {
	if (i == 2) p <- plot.func.fixed(r.m, var = names(r.m)[i], tit = tits[i], 
		breaks = b.ystar)
	if (i == 4) p <- plot.func.fixed4(r.m, var = names(r.m)[i], tit = tits[i], 
		breaks = b.error)
	if( i==1 | i == 3 | i == 5) 
...
for (i in 1:dim(r.e)[3]) {
	if (i == 2) p <- plot.func.fixed(r.e, var = names(r.e)[i], tit = tits[i], 
		breaks = b.ystar)
	if (i == 4) p <- plot.func.fixed4(r.e, var = names(r.e)[i], tit = tits[i], 
		breaks = b.error)
	if( i==1 | i == 3 | i == 5) 
		p <- plot.func(r.e, var = names(r.e)[i], tit = tits[i]) 
	assign(paste0("p",i), p)
}
png(filename = "F2e.png", w = 9*1.54, h = 2.3*1.2, units = "in", res = 600)
pushViewport(viewport(layout=grid.layout(1,5)))
print(p1, vp=viewport(layout.pos.col = 1))
print(p2, vp=viewport(layout.pos.col = 2))
print(p3, vp=viewport(layout.pos.col = 3))
...
for (i in 1:dim(r.gm)[3]) {
	if (i == 2) p <- plot.func.fixed(r.gm, var = names(r.gm)[i], tit = tits[i], 
		breaks = b.ystar)
	if (i == 4) p <- plot.func.fixed4(r.gm, var = names(r.gm)[i], tit = tits[i], 
		breaks = b.error)
	if( i==1 | i == 3 | i == 5) 
...
for (i in 1:dim(r.m)[3]) {
	if (i == 2) p <- plot.func.fixed(r.i, var = names(r.i)[i], tit = tits[i], 
		breaks = b.ystar)
	if (i == 5) p <- plot.func.fixed4(r.i, var = names(r.i)[i], tit = tits[i], 
		breaks = b.error)
	if( i==1 | i == 3 | i == 4) 
...
rr <- raster(rc); rr  <- setValues(rr,  df3$Reference)	
...
## 1. Load Data 
source_data("https://github.com/lexcomber/SematicsOfStats/blob/master/lp.RData?raw=True")
